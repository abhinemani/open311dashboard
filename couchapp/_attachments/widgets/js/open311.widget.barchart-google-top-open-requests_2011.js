...
$.widget('Open311.barchartGoogleTopOpenRequests', $.Open311.barchartGoogle, {
   ...
   */
  options: {
    title: 'Top Ten Service Requests in 2011',
    dataSource: "http://ec2-50-17-130-61.compute-1.amazonaws.com:5984/service-requests/_design/requests/_view/service_counts_2011?group=true",
    topX: 10
  },
    ...
    var self = this;
		...
		if(data.rows.length > 0) {
			...
			for (var i = 0; i < data.rows.length; i++){
			    totalServiceCount = totalServiceCount + data.rows[i].value;
			}
			...
			*/
			...
			var service_counts = data.rows;
			var topTen = service_counts.sort(function(a,b){ //pass in an anonymous function
				return b.value - a.value; //defines increasing order in the reverse: high to low
			});
		    ...
		    $(topTen).each(function(index, el) {
				...
				var requestType = el; //why is this declared here?
			        dataPar.push((requestType.value/totalServiceCount)*100);
			        axis.push(requestType.key);
			        dataLabels.push("t+" + requestType.value + ',676767,0,' + index + ',11');
			        // find the largest value for the xmax caluclation
			        maxValue = maxValue < requestType.value ? requestType.value : maxValue;
				    }
      		});
